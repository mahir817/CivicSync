...
import { campaignApi, civicReportApi, donationApi, profileApi } from '../api/client';
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    Promise.all([profileApi.get(), campaignApi.getMine(), donationApi.getMine(), donationApi.getPendingReceipts(), donationApi.getPledges(), civicReportApi.getMine()])
      .then(([p, c, d, r, pl, cr]) => { if (active) { setProfile(p.data); setForm(p.data); setPosts(c.data); setSupport(d.data); setReceipts(r.data); setPledges(pl.data); setReports(cr.data); } })
  ...
  return <Page title={t('profile')} subtitle={profile?.email}>
    ...
    {loading ? <Spinner /> : profile && <div className="space-y-7">
      ...
      <section><h2 className="mb-4 text-2xl font-bold">{t('yourReports')}</h2>{reports.length === 0 ? <Card>{t('empty')}</Card> : <div className="space-y-3">{reports.map(report => <Link to={'/civic-reports/' + report.id} key={report.id}><Card className="mb-3 flex items-center justify-between gap-3 hover:shadow-md"><div><p className="font-bold">{report.description}</p><p className="text-xs text-slate-500">{fmtDate(report.createdAt)}</p></div><Badge status={report.status}/></Card></Link>)}</div>}</section>
