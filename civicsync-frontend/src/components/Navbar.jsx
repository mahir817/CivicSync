...
export default function Navbar() {
  ...
  const linkClass = ({ isActive }, expanded = false) => (expanded ? 'min-w-0 rounded-xl break-words text-left ' : 'rounded-full ') + 'px-3 py-2 text-sm font-medium transition ' + (isActive ? 'bg-blue-50 text-blue-800' : 'text-slate-700 hover:bg-slate-100 hover:text-blue-800');
  ...
  return <header className="fixed left-0 top-0 z-[1000] w-full px-3 pt-3 sm:pt-5">
    <nav aria-label={t('menu')} className={'mx-auto max-w-7xl border border-slate-200 bg-white/95 px-3 py-2 text-slate-900 shadow-xl backdrop-blur-md ' + (open ? 'rounded-2xl xl:rounded-full' : 'rounded-3xl sm:rounded-full')}>
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <button onClick={() => setOpen(!open)} className={'p-2 xl:hidden ' + (open ? 'rounded-xl bg-blue-50' : 'rounded-full')} aria-label={t('menu')} aria-expanded={open}>{open ? <X size={22}/> : <Menu size={22}/>}</button>
        </div>
      </div>
      {open && <div className="mt-3 border-t border-slate-200 pt-3 xl:hidden">
        ...
        <div className="grid grid-cols-2 gap-2">{[...links, ...(user ? [['/inbox','inbox'], ['/profile','profile']] : []), ...roleLinks].map(([to, key]) => <NavLink key={to} to={to} onClick={() => setOpen(false)} className={state => linkClass(state, true)}>{t(key)}</NavLink>)}{user ? <button onClick={logout} className="rounded-xl bg-blue-50 px-3 py-2 text-left text-sm text-blue-800 sm:hidden">{t('logout')}</button> : <Link to="/login" onClick={() => setOpen(false)} className="rounded-xl bg-blue-50 px-3 py-2 text-sm text-blue-800 sm:hidden">{t('login')}</Link>}</div>
      </div>}
    </nav>
  </header>;
}
