...
These browser tests cover the shared navbar, English/Bengali switch, map coordinates, registration, verifier approval, offline contribution receipt, nearby blood donors, the donor countdown, civic confirmation limits, and admin dispute handling. They create accounts and requests, so **run them only against a disposable MySQL database**. The development seed supplies the example verifier, requester, and donor accounts.
